...
pub mod ierc777 {
  ...
  impl Methods {
    ...
    pub fn default_operators(
      &self,
    ) -> self::ethcontract::dyns::DynMethodBuilder<Vec<self::ethcontract::Address>> {
      self
        .instance
        .method([6, 228, 133, 56], ())
        .expect("generated call")
    ...
    pub fn is_operator_for(
      ...
      self
        .instance
        .method([217, 91, 99, 113], (operator, token_holder))
        .expect("generated call")
    }
    #[doc = "Generated by `ethcontract`"]
  ...
  pub mod event_data {
    ...
    impl self::ethcontract::web3::contract::tokens::Detokenize for AuthorizedOperator {
      fn from_tokens(
        ...
        let token_holder = < self :: ethcontract :: Address as self :: ethcontract :: web3 :: contract :: tokens :: Tokenizable > :: from_token (tokens . next () . unwrap ()) ? ;
    ...
    impl self::ethcontract::web3::contract::tokens::Detokenize for Burned {
      fn from_tokens(
        tokens: Vec<self::ethcontract::common::abi::Token>,
      ) -> Result<Self, self::ethcontract::web3::contract::Error> {
        if tokens.len() != 5 {
        ...
        let operator_data =
          <Vec<u8> as self::ethcontract::web3::contract::tokens::Tokenizable>::from_token(
            tokens.next().unwrap(),
          )?;
        Ok(Burned {
          operator,
    ...
    impl self::ethcontract::web3::contract::tokens::Detokenize for Minted {
      fn from_tokens(
        tokens: Vec<self::ethcontract::common::abi::Token>,
      ) -> Result<Self, self::ethcontract::web3::contract::Error> {
        if tokens.len() != 5 {
    ...
    impl self::ethcontract::web3::contract::tokens::Detokenize for Sent {
      fn from_tokens(
        tokens: Vec<self::ethcontract::common::abi::Token>,
      ) -> Result<Self, self::ethcontract::web3::contract::Error> {
        if tokens.len() != 6 {
          return Err(self::ethcontract::web3::contract::Error::InvalidOutputType(
            format!("Expected {} tokens, got {}: {:?}", 6, tokens.len(), tokens),
          ));
        }
        #[allow(unused_mut)]
        let mut tokens = tokens.into_iter();
    ...
    impl self::ethcontract::web3::contract::tokens::Detokenize for RevokedOperator {
      fn from_tokens(
        tokens: Vec<self::ethcontract::common::abi::Token>,
      ) -> Result<Self, self::ethcontract::web3::contract::Error> {
        if tokens.len() != 2 {
